...
const MovieDetails = () => {
  ...
  const { data: movie, loading: movieDetailsLoading } = useFetch(() =>
  ...
  useEffect(() => {
    const checkingLikeStatus = async () => {
      ...
      try {
        ...
        if (movie) {
          const status = await checkLikedStatus(user.$id, movie.id);
          setIsLiked(status);
        }
      } catch (error) {
    ...
    try {
      if (movie) {
        ...
        if (!prevLikeState) {
          ...
          const success = await removeFromSavedMovies(user.$id, movie.id);
          if (!success)
      ...
      >
        ...
        <View className="flex-col items-start justify-center mt-5 px-5">
          <View className="flex-row justify-between items-center flex-wrap">
            ...
            <View style={{ width: 50, height: 50 }}>
              ...
              <Heart
                ...
                isLiked={isLiked}
                isLoading={likeOperationLoading}
          ...
          <MovieItem
            ...
            content={
              movie?.production_countries.map((c) => c.name).join(" - ") || "NA"
            }
          />
        </View>
      </ScrollView>
